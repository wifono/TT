...
* @url http://members.chello.at/~easyfilter/barcode.html
...
function quickresponse(text,level,ver) { // create QR and micro QR bar code symbol
	...
	/** data analysis */
	for (mode = i = 0; i < text.length; i++) { // compute mode
		c = text.charCodeAt(i);
		...
		if (k1.indexOf(String.fromCharCode(c),k1.length>>1) < 0) { mode = 2; break; }
		mode = 3; // Kanji mode
	}
	...
	else for (i = 0; i < text.length; i++) { // encode Kanji
